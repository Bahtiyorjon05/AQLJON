...
class WordGenerator(BaseDocumentGenerator):
    ...
    async def generate(self, update: Update, context: ContextTypes.DEFAULT_TYPE, topic: str, content_context: str = ""):
        ...
        try:
            ...
            CRITICAL: Structure for VISUAL IMPACT with tables, data, and structured content:

            1. INCLUDE TABLES (mark with [TABLE]):
               [TABLE: Title]
               | Header 1 | Header 2 | Header 3 |
               | Data 1 | Data 2 | Data 3 |
               | Data 4 | Data 5 | Data 6 |

            2. INCLUDE DATA FOR CHARTS (mark with [CHART]):
               [CHART: Type - Title]
               Data: Label1: Value1, Label2: Value2, Label3: Value3

            3. STRUCTURE:
               - Main title (#): Compelling professional title
               - 4-6 sections (##): Clear descriptive headings
               - 2-3 subsections (###) per section
               - Mix text, tables, lists, data
            ...
            5. Analyze the topic and create well-structured engaging content that:
               - Provides medium-detailed coverage (not too brief, not overly detailed)
               - Uses tables for comparisons and structured data
               - Includes specific numbers, statistics, percentages
               - Incorporates bullet points, numbered lists, examples
            ...
            try:
                ...
                logger.error(f"Word content generation failed after retries: {e}")
                content = f"# {cleaned_topic}\n\nProfessional Word document on {cleaned_topic}.\n\nGenerated by AQLJON."
